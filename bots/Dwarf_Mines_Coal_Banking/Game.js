...
async function goToStartingPoint() {
  const coalMinimapLocation = await Minimap.getCoalMapLocation();
  if (coalMinimapLocation.length > 0) return;

  const aboveGroundLadderLocation = await AboveGroundLadder.getAboveGroundLadderLocation();
  if (aboveGroundLadderLocation.length > 0) {
    await AboveGroundLadder.goUnderground();
    return;
  }

  const bankBoothLocation = await Bank.getBankBoothLocation();
  if (bankBoothLocation.length > 0) {  
    await Bank.depositItemsInBank();
    await Bank.goFromBankToUndergound();
    return;
  }
